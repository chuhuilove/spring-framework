...
/**
 * {@link org.springframework.context.ApplicationContext}接口的抽象实现,基于模板模式
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			 ...
			 * 1.
			 * 设置一个Ioc容器的状态:
			 * 活跃状态设为true,关闭状态设置为false,启动时间,设置监听器等
			 */
			prepareRefresh();
			...
			try {
				 ...
				 */
				registerListeners();
				logger.debug("refresh第九次调用");
				getBeanFactoryContent(obtainContentFactory);

				// Instantiate all remaining (non-lazy-init) singletons.
				/**
				 * 11. 初始化所有剩余的单例bean(非懒加载)
				 *  我们的bean,就是在这里进行初始化的
				 */
				finishBeanFactoryInitialization(beanFactory);
				logger.debug("refresh第十次调用");
				getBeanFactoryContent(obtainContentFactory);
	...
	private void getBeanFactoryContent(DefaultListableBeanFactory beanFactory) {

		Map<String, Object> singletonObjects = beanFactory.getSingletonObjects();

		logger.trace("=================================BeanDefinition");
		beanFactory.getBeanDefinitionMap().forEach((key, value) -> logger.trace(key + "--->" + value.getBeanClassName()));
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		/**
		 * CONVERSION_SERVICE_BEAN_NAME 的值为conversionService
		 * 设定一个转换服务,
		...
		/**
		 * 如果以前没有任何bean后处理器注册过,则注册一个默认的嵌入式值解析器:
		 * 此时,主要用于注解属性值的解析
		 */
	...
	/**
	 * 获取bean的时候,需要判断Ioc容器是否还处于活跃状态.是否已经关闭了
	 * <p>
