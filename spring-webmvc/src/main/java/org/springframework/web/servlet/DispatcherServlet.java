...
/**
 * spring mvc 最核心的控制器下图,从上到下,是整个Spring MVC的运行流程
 * <p>
 * +-------------------+
 * + DispatcherServlet +
 * +-------------------+
...
public class DispatcherServlet extends FrameworkServlet {
	 ...
	 * <p>
	 * Spring mvc中各个组件的默认实现,配置在{@code DispatcherServlet.properties }文件中
	 */
	...
	static {
		...
		/**
		 * 加载spring mvc 各个组件的默认实现
		 * 这里只是将全类名加载进来,
		 * 而没有进一步将其实现.
		 *
		 * 具体实现,参看{@link #initStrategies(ApplicationContext)}中的注释
		 *
		 */
		//
		//
	...
	/**
	 * 这个地方是具体初始化Spring MVC 各个组件的函数.其调用过程,从上到下为:
	 * <p>
	 * tomcat--->{@link HttpServletBean#init()}---> {@link HttpServletBean#initServletBean()}
	 * 因为{@code HttpServletBean}中的{@code initServletBean} 是一个空方法,所以其具体实现在{@link FrameworkServlet#initServletBean()}
	 * ---->{@link FrameworkServlet#initWebApplicationContext()}--->{@link FrameworkServlet#onRefresh(ApplicationContext)}
	 * 同样,{@code FrameworkServlet}中的{@code onRefresh}也是一个空方法,其具体实现就到了本类{@link #onRefresh(ApplicationContext)}
	 * 本类中,{@code onRefresh}方法中只有一个函数调用,调用的就是本函数{@code initStrategies}
	 * <p>
	...
	private void initMultipartResolver(ApplicationContext context) {
		/**
		 *  在spring mvc 中上传文件,需要定义一个{@link MultipartResolver} bean
		 *  而beanName必须是multipartResolver,原因是就是在这里
		 *
		 *  从Bean工厂中获取beanName为{@link #MULTIPART_RESOLVER_BEAN_NAME}的实例.
		 *  从代码的角度来看,若不手动添加上传文件的组件,则不会设置默认的上传组件
		 */
	...
	private void initLocaleResolver(ApplicationContext context) {
		...
		/**
		 * 设置本地化解析器,和{@link #initMultipartResolver}差不多,在自定义的时候,也是需指定beanName的名字,{@link #LOCALE_RESOLVER_BEAN_NAME}
		 * 但是有一点不同的是,若是没有手动指定,则会加载一个默认值.
		 * 这个默认值,就存在于{@code DispatcherServlet}中的静态代码块中加载的{@link DEFAULT_STRATEGIES_PATH}中
		 *
		 * 默认值是{@link AcceptHeaderLocaleResolver}
		 *
		 */
