...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	...
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		...
		/**
		 * 给{@code HandlerExecutionChain}添加拦截器,
		 * 那么问题来了,这里的{@link adaptedInterceptors}是在哪来的呢?
		 * 在{@link WebMvcConfigurationSupport#getInterceptors()}这个函数中生成拦截器
		 * ,而在{@link WebMvcConfigurationSupport}中,每次生成一个{@code HandlerMapping},
		 * 都会调用一次{@code WebMvcConfigurationSupport#getInterceptors()}.
		 * 这就是本类中{@link adaptedInterceptors}数据的来源
		 *
		 */
