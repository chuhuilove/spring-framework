 ...
 *
 * 通过使用{@link HttpMessageConverter HttpMessageConverters}从request的body中读取内容来解析方法参数值的抽象类
 *
 *
 *
...
public abstract class AbstractMessageConverterMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	protected <T> Object readWithMessageConverters(HttpInputMessage inputMessage, MethodParameter parameter,
		...
		try {
			...
			logger.info("client request contextType is:"+contentType.toString());
			for (int i = 0; i < messageConverters.size(); i++) {
				HttpMessageConverter<?> converter = messageConverters.get(i);
				List<MediaType> supportedMediaTypes = converter.getSupportedMediaTypes();
				MediaType [] types=new MediaType[supportedMediaTypes.size()];
				supportedMediaTypes.toArray(types);


				logger.info("第" + (i + 1) + "个转换器尝试,该转换器的class为:" + converter.getClass().getName()+" support mediaType is"+
